...
One could use an in-memory solution and set my timer based on whether or not an image was selected. For instance, start all images out with a 5m deletion timer, but if the user selects the `circle` shape image then update its timer to last an extra 15 minutes since this is the one the user preferred, and continue doing this through each step of my app - add time to the selected image, and make sure any unselected images are set to be deleted in 5 minutes.
